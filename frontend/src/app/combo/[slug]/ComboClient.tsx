    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <section className="max-w-4xl mx-auto px-4 py-8 md:py-16 w-full flex-1 flex flex-col gap-8">
        ...
        <div className="p-6 md:p-8 rounded-3xl bg-gradient-to-br from-red-950/20 via-neutral-900/40 to-transparent border border-neutral-900 shadow-xl space-y-4">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight leading-none text-neutral-100">
            {combo.name}
          </h2>
          <p className="text-sm text-neutral-400 font-medium leading-relaxed max-w-2xl">
            {combo.description ||
        ...
        <div className="space-y-8">
          {(combo.comboItems || []).map((slot: ComboSlot) => {
            ...
            return (
              <div
                key={slot.slotId}
                className="p-5 rounded-3xl bg-neutral-900/20 border border-neutral-900/60 shadow-lg space-y-4"
              >
                <div className="flex items-center justify-between border-b border-neutral-900 pb-3">
                  ...
                  <span className="text-[10px] font-extrabold text-neutral-500 bg-neutral-900 px-2.5 py-1 rounded-full uppercase">
                    Required Selection
                  </span>
                </div>
                  ...
                  {candidateProducts.map((p) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group p-3 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between items-center text-center relative overflow-hidden ${
                          isSelected
                            ? 'border-red-600 bg-red-600/10 text-white font-bold shadow-md'
                            : 'border-neutral-900 bg-neutral-950/60 text-neutral-400 hover:border-neutral-800 hover:text-white hover:bg-neutral-900/30'
                        }`}
                      >
                        {isSelected && (
                          <span className="absolute top-2 right-2 p-0.5 rounded-full bg-red-600 text-white shadow">
                            <Check className="h-2.5 w-2.5 stroke-[4px]" />
                          </span>
                        )}
                        <div className="h-16 w-16 bg-neutral-900 border border-neutral-800/80 rounded-xl overflow-hidden mb-3 shrink-0">
                          <img
        ...
        {/* Floating Add Button */}
        <div className="p-6 rounded-3xl bg-neutral-900/40 border border-neutral-900 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
          <div>
          ...
          <Button
            ...
            className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white font-bold py-3.5 px-8 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-red-950/20 transition-all cursor-pointer"
          >
